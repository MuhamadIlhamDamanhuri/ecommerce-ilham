<?php
// Petugas/kelola_kategori_petugas.php
session_start();
...
// CRUD logic same as Admin version (Add, Update, Delete, Search)
// ... [Copy the same PHP logic from Admin/kelola_kategori.php here] ...
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_category'])) {
    ...
    if (empty($name)) { $error = 'Nama kategori wajib diisi'; }
    else {
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("SELECT id FROM categories WHERE name = :name");
            $stmt->execute(['name' => $name]);
            if ($stmt->fetch()) { $error = 'Kategori sudah ada'; }
            else {
...
// Handle Update/Delete similar to admin version...
// [Same logic as admin version]
...
$params = [];
if (!empty($search)) { $query .= " AND (name LIKE :search OR description LIKE :search)"; $params['search'] = '%'.$search.'%'; }
...
} catch (PDOException $e) { $error = '❌ Database error: ' . $e->getMessage(); $categories = []; $total = 0; $active = 0; }
        ...
        <div class="content-area">
            ...
            <div class="filter-section">
                <form method="GET" action="" class="filter-form">
                    ...
                    <div class="filter-group" style="min-width:auto;"><button type="submit" class="btn btn-primary">Filter</button><a href="kelola_kategori_petugas.php" class="btn btn-warning" style="margin-left:10px;">Reset</a></div>
                </form>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        <?php if(count($categories) > 0): ?>
                            ...
                            <?php $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM products WHERE category_id = :id"); $stmt->execute(['id' => $c['id']]); $product_count = $stmt->fetch()['count']; ?>
                            <tr>
                                <td><?php echo $c['id']; ?></td>
