...
try {
    ...
    if ($report_type === 'transaksi') {
        $query = "SELECT t.*, u.username, u.full_name, u.email FROM transactions t LEFT JOIN users u ON t.user_id = u.id WHERE 1=1";
        $params = [];
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) as total_orders, SUM(total_amount) as total_revenue, AVG(total_amount) as avg_order FROM transactions WHERE created_at BETWEEN :from AND :to");
        $stmt->execute(['from' => $date_from . ' 00:00:00', 'to' => $date_to . ' 23:59:59']);
        $summary = $stmt->fetch();
        
    } elseif ($report_type === 'penjualan') {
        $query = "SELECT DATE(t.created_at) as sale_date, COUNT(t.id) as orders, SUM(t.total_amount) as revenue, SUM(td.quantity) as items_sold FROM transactions t LEFT JOIN transaction_details td ON t.id = td.transaction_id WHERE t.payment_status = 'paid'";
        $params = [];
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) as total_orders, SUM(total_amount) as total_revenue, AVG(total_amount) as avg_order FROM transactions WHERE payment_status = 'paid' AND created_at BETWEEN :from AND :to");
        $stmt->execute(['from' => $date_from . ' 00:00:00', 'to' => $date_to . ' 23:59:59']);
        $summary = $stmt->fetch();
        ...
        $stmt = $pdo->query("SELECT COUNT(*) as total_products, SUM(stock) as total_stock, AVG(stock) as avg_stock, SUM(CASE WHEN stock <= 5 THEN 1 ELSE 0 END) as low_stock FROM products WHERE status = 'active'");
        $summary = $stmt->fetch();
    }
        ...
        <div class="content-area">
            ...
            <div class="report-controls">
                <form method="GET" style="display:flex;gap:15px;flex-wrap:wrap;align-items:flex-end;flex:1;">
                    ...
                    <div class="form-group"><label>Dari Tanggal</label><input type="date" name="date_from" value="<?php echo htmlspecialchars($date_from); ?>"></div>
                    <div class="form-group"><label>Sampai Tanggal</label><input type="date" name="date_to" value="<?php echo htmlspecialchars($date_to); ?>"></div>
                    <?php endif; ?>
                    
                    <div class="form-group"><label>Cari</label><input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Cari..."></div>
                    
                    <?php if ($report_type === 'transaksi'): ?>
                    <div class="form-group"><label>Status</label><select name="status"><option value="">Semua</option><option value="pending" <?php echo $status_filter === 'pending' ? 'selected' : ''; ?>>Pending</option><option value="verified" <?php echo $status_filter === 'verified' ? 'selected' : ''; ?>>Verified</option><option value="processing" <?php echo $status_filter === 'processing' ? 'selected' : ''; ?>>Processing</option><option value="shipped" <?php echo $status_filter === 'shipped' ? 'selected' : ''; ?>>Shipped</option><option value="completed" <?php echo $status_filter === 'completed' ? 'selected' : ''; ?>>Completed</option><option value="cancelled" <?php echo $status_filter === 'cancelled' ? 'selected' : ''; ?>>Cancelled</option></select></div>
                    <?php elseif ($report_type === 'stok'): ?>
                    <div class="form-group"><label>Status</label><select name="status"><option value="">Semua</option><option value="active" <?php echo $status_filter === 'active' ? 'selected' : ''; ?>>Aktif</option><option value="inactive" <?php echo $status_filter === 'inactive' ? 'selected' : ''; ?>>Nonaktif</option></select></div>
                    <?php endif; ?>
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card"><h4>Total Order</h4><div class="value"><?php echo number_format($summary['total_orders'] ?? 0); ?></div></div>
                <div class="stat-card success"><h4>Total Pendapatan</h4><div class="value">Rp <?php echo number_format($summary['total_revenue'] ?? 0, 0, ',', '.'); ?></div></div>
                <div class="stat-card"><h4>Rata-rata Order</h4><div class="value">Rp <?php echo number_format($summary['avg_order'] ?? 0, 0, ',', '.'); ?></div></div>
                <?php elseif ($report_type === 'stok'): ?>
                <div class="stat-card"><h4>Total Produk</h4><div class="value"><?php echo number_format($summary['total_products'] ?? 0); ?></div></div>
                <div class="stat-card success"><h4>Total Stok</h4><div class="value"><?php echo number_format($summary['total_stock'] ?? 0); ?></div></div>
                <div class="stat-card"><h4>Rata-rata Stok</h4><div class="value"><?php echo number_format($summary['avg_stock'] ?? 0, 0); ?></div></div>
                <div class="stat-card warning"><h4>Stok Menipis</h4><div class="value"><?php echo number_format($summary['low_stock'] ?? 0); ?></div></div>
                <?php endif; ?>
            </div>
            <?php endif; ?>
            ...
            <div class="table-container">
                <table>
                    <?php if ($report_type === 'transaksi'): ?>
                    <thead><tr><th>No. Order</th><th>Pelanggan</th><th>Total</th><th>Metode</th><th>Bayar</th><th>Status</th><th>Tanggal</th></tr></thead>
                    <tbody>
                    ...
                    <thead><tr><th>Tanggal</th><th>Jumlah Order</th><th>Item Terjual</th><th>Pendapatan</th></tr></thead>
                    <tbody>
                        <?php if (!empty($report_data)): ?>
                            <?php foreach ($report_data as $row): ?>
                            <tr>
                    ...
                    <?php elseif ($report_type === 'stok'): ?>
                    <thead><tr><th>ID</th><th>Produk</th><th>Kategori</th><th>Harga</th><th>Stok</th><th>Status Stok</th><th>Status</th></tr></thead>
                    <tbody>
                        <?php if (!empty($report_data)): ?>
                            <?php foreach ($report_data as $row): ?>
                            <?php
